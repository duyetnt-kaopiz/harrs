...
import { isHTMLElement, isValidDate, validateArrayType, validateProps, validateRowsPerPage, } from "../base/validator";
import "../base/pagination";
import { READ_ONLY_TABLE_CSS } from "./style";
let exportReadOnlyTable;
(() => {
    exportReadOnlyTable = window.customElements.get("kuc-readonly-table-1-23-1");
    ...
    class KucReadOnlyTable extends KucBase {
        ...
        _createDisplayData() {
            ...
            if (this._sortField && this._sortDirection) {
                displayData = this._sortData(displayData, this._sortField, this._sortDirection);
            }
            if (!this.pagination)
                return displayData;
            const firstRow = (this._pagePosition - 1) * this.rowsPerPage + 1;
            const lastRow = this._pagePosition * this.rowsPerPage;
            return displayData.filter((_element, index) => index >= firstRow - 1 && index <= lastRow - 1);
        }
        ...
        _handleClickHeader(field) {
            this._sortFields(field);
        }
        _handleKeyDownHeader(event, field) {
        ...
        _getColumnsTemplate(column, index) {
            ...
            const sortClass = isSorted
                ? ` kuc-readonly-table-1-23-1__table__header__cell--sorted-${this._sortDirection}`
                : "";
            return html `
        <th
          class="kuc-readonly-table-1-23-1__table__header__cell${isHTML
                ...
                ? " kuc-readonly-table-1-23-1__table__header__cell--first-visible"
                : ""}${isLastVisible
                ? " kuc-readonly-table-1-23-1__table__header__cell--last-visible"
                : ""}"
      ...
      `;
        ...
        _getDataTemplate(data, currentIndex) {
            return html `
        <tr
          class="kuc-readonly-table-1-23-1__table__body__row kuc-readonly-table-1-23-1__table__body__row-${currentIndex}"
        >
          ${this._columnOrder.map((currentCol, colIndex) => {
              ...
              >
                ...
                return html `<td class="kuc-readonly-table-1-23-1__table__body__row__cell-data" ?hidden="${!visible}" style="width: ${customWidth}; min-width: ${customWidth}; max-width: ${customWidth}">${value}</td>`;
            })}
        </tr>
      `;
        }
    ...
    __decorate([
        state()
    ], KucReadOnlyTable.prototype, "_sortField", void 0);
    __decorate([
        state()
    ], KucReadOnlyTable.prototype, "_sortDirection", void 0);
    window.customElements.define("kuc-readonly-table-1-23-1", KucReadOnlyTable);
